...
opendelete:boolean=false;
  ...
  validation() {
    ...
    else if (this.Name && this.Phone && this.Salary && this.Section && this.Gender  ) {
  ...
  clearform() {
   ...
   deleteon(data: string ) {
    this.opendelete=!this.openedit;
...
this.editsid=this.createStaffForm.value.SID;
  ...
  updatedata(){
      ...
      this.newstaff.updatestaff(this.temporaryvariable, this.createStaffForm.value).then(res=>{
        console.log(res);
        this.Save=res;
